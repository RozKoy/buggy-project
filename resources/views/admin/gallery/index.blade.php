...
@section('content')
    <!-- Breadcrumb -->
    <x-breadcrumb :breadcrumbs="[['title' => 'Home', 'url' => '#'], ['title' => 'GALLERY', 'url' => '#'], ['title' => 'List', 'url' => null]]" />
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow hover:shadow-lg transition-shadow">
                        <div class="relative">
                            <div class="h-48 overflow-hidden rounded-t-lg bg-gray-100">
                                <img class="w-full h-full object-cover" src="{{ asset('storage/' . $gallery->image) }}" loading="lazy" alt="image">
                            </div>
                            <!-- Delete Icon Button -->
                            <form action="{{ route('admin.gallery.destroy', $gallery->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this data?');" class="absolute top-2 right-0 mr-2">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 bg-red-600 hover:bg-red-700 rounded-lg shadow-lg transition-colors group">
    ...
    <!-- Main modal -->
    <div id="default-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-screen bg-opacity-75">
