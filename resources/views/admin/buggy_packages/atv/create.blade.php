...
@section('content')
    <x-breadcrumb :breadcrumbs="[['title' => 'Home', 'url' => '#'], ['title' => 'ATV', 'url' => '/atv'], ['title' => 'create', 'url' => null]]" />
    <x-form id="submit_form" header="Add UTV" action="{{ route('admin.atv.store') }}">
        ...
        <div class="mb-6">
            ...
            <x-input-field type="text" id="name" name="name" placeholder="SINGLE ATV..." value="{{ old('name') }}" required />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        <div class="mb-6">
            <x-label for="spec">Specification</x-label>
            <x-input-field type="text" id="spec" name="spec" placeholder="Two rides, one ATV..." value="{{ old('spec') }}" />
