...
@section('content')
    <x-breadcrumb :breadcrumbs="[['title' => 'Home', 'url' => '#'], ['title' => 'ATV', 'url' => '/atv'], ['title' => 'Edit', 'url' => null]]" />
    <x-form id="submit_form" header="Add ATV" action="{{ route('admin.atv.update', $atv->id) }}">
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <x-input-field type="text" id="name" name="name" placeholder="SINGLE ATV..." value="{{ old('name', $atv->name) }}" required />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        <div class="mb-6">
            <x-label for="spec">Specification</x-label>
            <x-input-field type="text" id="spec" name="spec" placeholder="Two rides, one ATV..." value="{{ old('spec', $atv->spec) }}" />
        ...
        <div class="mb-6">
            ...
            <x-input-field type="text" id="price" name="price" placeholder="1.000.000" value="{{ number_format(old('price', $atv->price), 0, ',', '.') }}" />
...
@push('js')
    <script>
        ...
        const oldContent = `{!! old('description', $atv->description) !!}`;
        if (oldContent) {
