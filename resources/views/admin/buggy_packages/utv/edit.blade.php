...
@section('content')
    <x-breadcrumb :breadcrumbs="[['title' => 'Home', 'url' => '#'], ['title' => 'UTV', 'url' => '/utv'], ['title' => 'Edit', 'url' => null]]" />
    <x-form id="submit_form" header="Add UTV" action="{{ route('admin.utv.update', $utv->id) }}">
        ...
        <div class="mb-6">
            ...
            <x-input-field type="text" id="name" name="name" placeholder="SINGLE UTV..." value="{{ old('name', $utv->name) }}" required />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        <div class="mb-6">
            <x-label for="spec">Specification</x-label>
            <x-input-field type="text" id="spec" name="spec" placeholder="Two rides, one UTV..." value="{{ old('spec', $utv->spec) }}" />
        ...
        <div class="mb-6">
            ...
            <x-input-field type="text" id="price" name="price" placeholder="1.000.000" value="{{ number_format(old('price', $utv->price), 0, ',', '.') }}" />
            <x-input-error :messages="$errors->get('price')" class="mt-2" />
        </div>
        {{-- Footer --}}
        <x-card-footer>
            <a href="{{ route('admin.utv.index') }}"><x-button variant="light" class="mr-3">Cancel</x-button></a>
...
@push('js')
    <script>
        ...
        const oldContent = `{!! old('description', $utv->description) !!}`;
        if (oldContent) {
